...
const daddy = process.env.OWNER_NUMBER ? process.env.OWNER_NUMBER + "@s.whatsapp.net" : "254700000000@s.whatsapp.net";
...
async function handleDeletedMessage(client, revocationMessage) {
  ...
  let notificationText = `*😈 CYBER-X MD Anti-Delete 😈*\n\n` +
 ...
 /* client.ev.on('messages.upsert', async ({ messages }) => {
  ...
  client.ev.on("messages.upsert", async (chatUpdate) => {
    try {
      ...
      const cyberX = m.quoted || m;
      const quoted = cyberX.mtype === 'buttonsMessage' ? cyberX[Object.keys(cyberX)[1]] :
        cyberX.mtype === 'templateMessage' ? cyberX.hydratedTemplate[Object.keys(cyberX.hydratedTemplate)[1]] :
          cyberX.mtype === 'product' ? cyberX[Object.keys(cyberX)[0]] : m.quoted ? m.quoted : m;
