...
function smsg(CyberXInstance, message, store) {
  if (!message) {
    return message;
  }
  ...
  if (message.key) {
    ...
    message.sender = CyberXInstance.decodeJid(
      message.fromMe && CyberXInstance.user.id || message.participant || message.key.participant || message.chat || ''
    );
    
    if (message.isGroup) {
      message.participant = CyberXInstance.decodeJid(message.key.participant) || '';
    }
  }
  ...
  if (message.message) {
    ...
    if (quotedMessage) {
      ...
      message.quoted.sender = CyberXInstance.decodeJid(message.msg.contextInfo.participant);
      message.quoted.fromMe = message.quoted.sender === CyberXInstance.decodeJid(CyberXInstance.user.id);
      ...
      message.quoted.getQuotedObj = message.quoted.getQuotedMessage = async () => {
        ...
        let quotedMsg = await store.loadMessage(message.chat, message.quoted.id, CyberXInstance);
        return smsg(CyberXInstance, quotedMsg, store);
      };
      ...
      message.quoted["delete"] = () => CyberXInstance.sendMessage(message.quoted.chat, { 'delete': quotedMessageFakeObj.key });
      message.quoted.copyNForward = (toChat, forceForward = false, options = {}) => CyberXInstance.copyNForward(toChat, quotedMessageFakeObj, forceForward, options);
      message.quoted.download = () => CyberXInstance.downloadMediaMessage(message.quoted);
    }
  }

  // Handle URL in the message
  if (message.msg && message.msg.url) {
    message.download = () => CyberXInstance.downloadMediaMessage(message.msg);
  }

  // Extract main text content from the message
  message.text = message.msg ? (message.msg.text || message.msg.caption || message.message.conversation || message.msg.contentText || message.msg.selectedDisplayText || message.msg.title || '') : '';

  // Reply function
  message.reply = (replyText, replyTo = message.chat, options = {}) => {
    if (Buffer.isBuffer(replyText)) {
      return CyberXInstance.sendMedia(replyTo, replyText, "file", '', message, { ...options });
    }
    return CyberXInstance.sendText(replyTo, replyText, message, { ...options });
  };

  // Function to copy the message
  message.copy = () => smsg(CyberXInstance, messageInfo.fromObject(messageInfo.toObject(message)), store);

  // Function to forward the message
  message.copyNForward = (toChat = message.chat, forceForward = false, options = {}) => CyberXInstance.copyNForward(toChat, message, forceForward, options);
