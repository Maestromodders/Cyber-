...
function smsg(CYBER-XInstance, message, store) {
  if (!message) {
    return message;
  }
  ...
  if (message.key) {
    ...
    message.sender = CYBER-XInstance.decodeJid(
      message.fromMe && CYBER-XInstance.user.id || message.participant || message.key.participant || message.chat || ''
    );
    
    if (message.isGroup) {
      message.participant = CYBER-XInstance.decodeJid(message.key.participant) || '';
    }
  }
  ...
  if (message.message) {
    ...
    if (quotedMessage) {
      ...
      message.quoted.sender = CYBER-XInstance.decodeJid(message.msg.contextInfo.participant);
      message.quoted.fromMe = message.quoted.sender === CYBER-XInstance.decodeJid(CYBER-XInstance.user.id);
      ...
      message.quoted.getQuotedObj = message.quoted.getQuotedMessage = async () => {
        ...
        let quotedMsg = await store.loadMessage(message.chat, message.quoted.id, CYBER-XInstance);
        return exports.smsg(CYBER-XInstance, quotedMsg, store);
      };
      ...
      message.quoted["delete"] = () => CYBER-XInstance.sendMessage(message.quoted.chat, { 'delete': quotedMessageFakeObj.key });
      message.quoted.copyNForward = (toChat, forceForward = false, options = {}) => CYBER-XInstance.copyNForward(toChat, quotedMessageFakeObj, forceForward, options);
      message.quoted.download = () => CYBER-XInstance.downloadMediaMessage(message.quoted);
    }
  }

  // Handle URL in the message
  if (message.msg.url) {
    message.download = () => CYBER-XInstance.downloadMediaMessage(message.msg);
  }
  ...
  message.reply = (replyText, replyTo = message.chat, options = {}) => {
    if (Buffer.isBuffer(replyText)) {
      return CYBER-XInstance.sendMedia(replyTo, replyText, "file", '', message, { ...options });
    }
    return CYBER-XInstance.sendText(replyTo, replyText, message, { ...options });
  };

  // Function to copy the message
  message.copy = () => exports.smsg(CYBER-XInstance, messageInfo.fromObject(messageInfo.toObject(message)));

  // Function to forward the message
  message.copyNForward = (toChat = message.chat, forceForward = false, options = {}) => CYBER-XInstance.copyNForward(toChat, message, forceForward, options);
